(()=>{
  ...
  const send=()=>{const p=product();if(!p.title)return;chrome.runtime.sendMessage({type:'PF_AMAZON_PRODUCT',product:p}).catch(()=>{});};
  chrome.runtime.onMessage.addListener((m,_s,reply)=>{if(m?.type==='PF_CAPTURE_AMAZON_NOW'){reply({ok:true,product:product()});return true;}});
})();